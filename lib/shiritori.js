...
/*original code is in anoriqq/u20-shiritori-bot-nodejs*/
...
module.exports = async function shiritori(message) {
  const [JUDG, parsedContent] = await judg1(message);
...
function judg1(message){
  ...
  return new Promise(async(resolve)=>{
    ...
    if(parsedContent.length === 1){
      judg = await judg2(message, judg, parsedContent[0]);
    }
    /* 文章の場合 NG */
    else{
      judg = 'B';
    }
    resolve([judg, parsedContent]);
  });
}

/* 名詞かそれ以外かを判定する関数 */
function judg2(message, judg, parsedContent){
  console.log('2次判定処理');
  return new Promise(async(resolve)=>{
    const lexical = parsedContent[1];
    /* 名詞の場合 OK */
    if(lexical === '名詞'){
      judg = await judg3(message, judg, parsedContent);
    }
    /* 名詞以外の場合 NG */
    else{
      judg = 'C';
...
/* 読み方が解析可能かどうかを判定する関数 */
function judg3(message, judg, parsedContent){
  console.log('3次判定処理');
  return new Promise(async(resolve)=>{
    const reading = parsedContent[8];
    /* 読み方が解析可能な場合 NG */
    if(reading !== undefined){
      judg = await judg4(message, judg, reading);
    }
    /* 読み方が解析不能な場合 NG */
    else{
      judg = 'D';
...
function judg4(message, judg, reading){
  console.log('4次判定処理');
  return new Promise(async(resolve)=>{
    ...
    if(linkingFlag === 'LINK'){
      judg = await judg5(message, judg, formattedReading);
    }
    /* 拗音リスト不足 NG */
...
function judg5(message, judg, formattedReading){
  console.log('5次判定処理');
  return new Promise(async(resolve)=>{
    /* 'ン'で終わらない場合 OK */
    if(formattedReading.last !== 'ン'){
      judg = await judg6(message, judg);
    }
    /* 'ン'で終わる場合 NG */
    else{
      judg = 'F';
...
function linking(first, last){
  return new Promise((resolve)=>{
    if(last === first){
      resolve('LINK');
    }else if(first.length === 2 && first.slice(0, 1) === last){
      resolve('LINK');
...
function testKogakimoji(word){
  const kogakimoji = ['ァ','ィ','ゥ','ェ','ォ','ヵ','ㇰ','ヶ','ㇱ','ㇳ','ㇴ','ㇵ','ㇶ','ㇷ','ㇸ','ㇹ','ㇺ','ャ','ュ','ョ','ㇻ','ㇼ','ㇽ','ㇾ','ㇿ','ヮ'];
